...
def build_report(result: "BacktestResult", out_path, *, store=None, extra: dict | None = None) -> Path:
    ...
    nav_stress = '<a href="#section-f">F. Crisis stress</a>' if stress is not None else ""
    nav = f"""
    <nav class="odds-nav">
    ...
    if stress is not None:
        comparisons = stress.get("comparisons")
        f_parts = [
            _block(js.embed(F.fig_stress_scenario_pnl(comparisons)), "Scenario P&L: defined vs undefined risk"),
            _block(js.embed(F.fig_stress_worst_position(comparisons)), "Worst single position per scenario"),
        ]
        for heading, fig in stress.get("odds_overlay_figs", []):
            f_parts.append(_block(js.embed(fig), heading))
        skipped = stress.get("skipped") or {}
        if skipped:
            skipped_html = "<ul>" + "".join(
                f"<li>{html.escape(str(name))}: {html.escape(str(reason))}</li>" for name, reason in skipped.items()
            ) + "</ul>"
            f_parts.append(f'<p class="odds-empty">Scenarios NOT built (never fabricated -- see reasons):</p>{skipped_html}')
        long_edge = stress.get("long_history_edge")
        if long_edge:
            f_parts.append(
                "<h3>Long-window empirical distribution (includes 2008) vs the run's default</h3>"
                f"<pre>{html.escape(json.dumps(long_edge, indent=2, default=str))}</pre>"
            )
        beta_sens = stress.get("beta_sensitivity")
        if beta_sens is not None and not beta_sens.empty:
            f_parts.append(
                "<h3>IV-mapping sensitivity (beta sweep, STRATEGY.md §10.2)</h3>"
                f'<div class="table-scroll">{beta_sens.to_html(index=False, classes="trade-table", border=0)}</div>'
            )
        section_f = f"""
        <section id="section-f" class="odds-section"><div class="container">
          <h2>F. Crisis stress (STRATEGY.md §10)</h2>
          <p style="color:var(--text-dim)">What the positions this run actually opened would have
          done under realized underlying paths the option-chain history does not cover.
          Positions are replayed with the SAME exit policy the run used; strikes and legs come
          from the actual trades, not a fresh selection.</p>
          {"".join(f_parts)}
        </div></section>
        """
